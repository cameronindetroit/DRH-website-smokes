...
| `src/utils/RandomUtil.ts` | none (internal) | Seeded RNG helper (`mulberry32`) and helper to pick distinct indexes for smoke selection; respects `SMK_SEED` env var |
| `src/utils/RotationUtil.ts` | none (internal) | Deterministic rotation index helper used by CI or teams preferring rotation-based picks |
| `src/utils/TestUtil.ts` | none (internal) | Test helper to attach screenshots and test artifacts to `test.info()` |
...
- `states.json` — Canonical state & city list used by smoke tests (generated by `scripts/scrape_drhorton_states.mjs`)

Helper scripts (development only):

- `scripts/scrape_drhorton_states.mjs` — Scrapes drhorton.com to generate `states.json` (used to bootstrap test data)
- `scripts/inspect_state_option.mjs` — Quick DOM inspector to save the state search dropdown for debugging selector issues
...
Current page objects (reflects repository state):

| Page Object | Represents |
|-------------|------------|
| `BasePage.ts` | Shared header/footer/nav/cookie-banner helpers |
| `HomePage.ts` | Home / landing page with community search |
| `WhoWeArePage.ts` | /who-we-are page |
| `SmartHomePage.ts` | /smart-home page |
| `ServicesPage.ts` | /services page |
| `CustomerCarePage.ts` | /customer-care page |
| `ContactUsPage.ts` | /contact-us page |
| `WarrantyPage.ts` | /warranty page |
| `MilitaryBenefitsPage.ts` | /military-benefits page |
| `CareersPage.ts` | /careers page |
| `ListingsPage.ts` | Centralized listing-related locators and helpers for state listings |
| `StateLandingPage.ts` | State landing pages (e.g. `/texas`) and local search helpers |
| `SettingsPage.ts` | Settings screen |
| `PageManager.ts` | Lazy-instantiated page object factory / orchestrator |
...
| Location | Notes |
|----------|-------|
| `src/tests/helpers/` | Folder for reusable test workflow helpers. In this branch the folder is present but contains no committed helper files — dashboard-specific helpers were removed as part of the smoke-selection refactor. |

Recommended helper responsibilities (examples you can add under `src/tests/helpers/`):

| Helper (example) | Purpose |
|------------------|---------|
| `DashboardAuthHelper.ts` | Authentication flows for dashboard tests (login, token refresh, session setup) |
| `E2EFlowHelper.ts` | Reusable multi-step user journeys used by E2E specs |
| `DataSetupHelper.ts` | Prepare test data (API seeds, DB fixtures) for regression/E2E runs |

Guidelines:

- Keep helpers focused: a helper should implement a small, reusable workflow (e.g., 'authenticate and navigate to dashboard').
- Expose only high-level steps (avoid exposing page locators directly — use PageObjects for that).
- Place helpers in `src/tests/helpers/` and import them from fixtures or spec files as needed.

### Test Organization

Tests are organized into tiers by scope; the repository currently contains `smoke`, `regression`, `e2e`, and `unit` test suites. Helpers live under `src/tests/helpers/` (currently empty in this branch).
...
└── unit/            ← utility/unit tests (e.g. `rotation.spec.ts`)
```

Notes:

- `smoke/` contains fast, independent checks used as deployment gates and is organized by functional area (home, navigation, pages, privacy, dashboard).
- `regression/` holds feature-level regression suites used for deeper validation on changes.
- `e2e/` contains longer user-journey tests that exercise multiple pages and integrations.
- `unit/` contains small, fast tests for utilities and core logic (for example, `src/tests/unit/rotation.spec.ts` verifies `RotationUtil` precedence and behavior).
- `src/tests/helpers/` is the place for reusable test workflow helpers; the folder is intentionally present even if helper files are not committed yet.
...
| ID | Test | Spec File |
|----|------|-----------|
| SMK-001 | Home page loads | `src/tests/smoke/home/homeSmoke.spec.ts` |
| SMK-002 | Logo renders on home page | `src/tests/smoke/home/homeSmoke.spec.ts` |
| SMK-003 | Main navigation links resolve | `src/tests/smoke/navigation/navigationSmoke.spec.ts` |
| SMK-004 | Who We Are page loads | `src/tests/smoke/pages/pageLoadSmoke.spec.ts` |
| SMK-005 | Smart Home page loads | `src/tests/smoke/pages/pageLoadSmoke.spec.ts` |
| SMK-006 | Services page loads | `src/tests/smoke/pages/pageLoadSmoke.spec.ts` |
| SMK-007 | Customer Care page loads | `src/tests/smoke/pages/pageLoadSmoke.spec.ts` |
| SMK-008 | Contact Us page loads | `src/tests/smoke/pages/pageLoadSmoke.spec.ts` |
| SMK-009 | Warranty page loads | `src/tests/smoke/pages/pageLoadSmoke.spec.ts` |
| SMK-010 | Military Benefits page loads | `src/tests/smoke/pages/pageLoadSmoke.spec.ts` |
| SMK-011 | Careers page loads | `src/tests/smoke/pages/pageLoadSmoke.spec.ts` |
| SMK-012 | Community search is present | `src/tests/smoke/home/homeSmoke.spec.ts` |
| SMK-013 | Footer renders on home page | `src/tests/smoke/home/homeSmoke.spec.ts` |
| SMK-014 | Cookie banner displays | `src/tests/smoke/privacy/cookieSmoke.spec.ts` |
| SMK-015 | Cookie banner dismissible | `src/tests/smoke/privacy/cookieSmoke.spec.ts` |
| SMK-016 | State landing page loads (Texas) | `src/tests/smoke/pages/pageLoadSmoke.spec.ts` |
| SMK-017 | Skip to main content link | `src/tests/smoke/home/homeSmoke.spec.ts` |
| SMK-018 | Top banner visible | `src/tests/smoke/home/homeSmoke.spec.ts` |
| SMK-020 | Search input navigates via random picks | `src/tests/smoke/home/homeSmoke.spec.ts` |
...
| SMK-020 | `selection-info`, `state-search-dropdown`, `state-listings` |
